...
import { Star, History, Lock, Search, Filter, ShieldCheck } from 'lucide-react';
import { fetchEvaluations, toggleLikeEvaluation, fetchGeneralSettings } from '../firebase';

export default function CommunityFeed({ user, onLogin, onBack, centersList }) {
    ...
    useEffect(() => {
        const loadFeed = async () => {
            ...
            const [data, loadedSettings] = await Promise.all([
                fetchEvaluations(user ? null : 3), // If not logged in, just fetch 3
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {filteredEvaluations.map((ev) => (
                        <div key={ev.id} className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
                            
                            {/* Watermark Logo if Verified */}
                            {ev.isAdminVerified && (
                                <div className="absolute -right-6 -top-6 w-24 h-24 bg-blue-50 rounded-full flex items-end justify-start p-4 opacity-50 pointer-events-none group-hover:scale-110 transition-transform">
                            ...
                            <div className="flex justify-between items-start mb-4 relative z-10">
                                <div className="flex items-center gap-3">
                                    {ev.userPhotoURL ? (
                                        <img src={ev.userPhotoURL} alt="Avatar" className="w-12 h-12 rounded-full border-2 border-white shadow-sm object-cover" />
                                    ) : (
                                        <div className="w-12 h-12 rounded-full bg-slate-100 text-slate-500 flex items-center justify-center font-bold text-lg border-2 border-white shadow-sm">
                                    ...
                                    <div>
                                        ...
                                        {ev.userLevel ? (
                                            <span 
                                                className="text-[10px] font-bold px-2 py-0.5 rounded-md flex items-center gap-1 mt-1 w-max border"
                                                style={{ backgroundColor: ev.userLevel.bg, color: ev.userLevel.color, borderColor: `${ev.userLevel.color}30` }}
                                            >
                            ...
                            {/* Evaluation Body */}
                            <div className="bg-slate-50/80 p-4 rounded-2xl mb-4 border border-slate-100 relative z-10">
                                <div className="flex justify-between items-start mb-3">
                                    ...
                                    <div className="flex items-center gap-1 bg-amber-50 px-2 py-1.5 rounded-lg shrink-0 border border-amber-100/50 shadow-sm">
                                ...
                                <div className="flex flex-wrap gap-2 items-center">
                                    ...
                                    <span className="text-[10px] font-bold text-slate-500 bg-white border border-slate-200 px-2.5 py-0.5 rounded-full shadow-sm">
                                        {ev.price}tr VNĐ
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="flex justify-between items-center text-xs text-slate-400 mt-2 pt-4 border-t border-slate-50 relative z-10">
                                ...
                                <button 
                                    ...
                                    className={`flex items-center gap-1.5 px-4 py-2 rounded-full font-bold transition-all hover:scale-105 active:scale-95 shadow-sm ${ev.likedBy?.includes(user?.uid) ? 'bg-red-50 text-red-600 ring-1 ring-red-200' : 'bg-white text-slate-500 hover:bg-slate-50 ring-1 ring-slate-200'}`}
                                >
