...
import { ArrowLeft, ArrowRight, Info, CheckCircle2, Star, Crown, MessageSquare } from 'lucide-react';
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start relative">
                ...
                <div className="lg:col-span-7 space-y-4">
                    {currentCriterion.type === 'text' ? (
                        <div className="bg-white border-2 border-slate-200 rounded-2xl p-6 shadow-sm">
                            <textarea
                                className="w-full h-48 bg-slate-50 border border-slate-200 rounded-xl p-4 text-slate-700 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all resize-none font-medium"
                                placeholder="Hãy chia sẻ thêm trải nghiệm thực tế của bạn tại trung tâm này nhé..."
                                value={currentAnswer || ''}
                                onChange={(e) => onSelectAnswer(e.target.value)}
                            ></textarea>
                            <p className="text-xs text-slate-400 mt-3 font-medium flex items-center gap-1.5">
                                <Info className="w-4 h-4" /> Đánh giá chi tiết của bạn sẽ giúp cộng đồng có thêm góc nhìn chân thực nhất.
                            </p>
                        </div>
                    ) : (
                ...
                <div className="lg:col-span-5 lg:sticky lg:top-8 w-full">
                    <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 sm:p-8 min-h-[360px] flex flex-col shadow-md transition-all duration-300 relative overflow-hidden">
                        ...
                        {currentCriterion.type === 'text' ? (
                            <div className="flex-1 flex flex-col items-center justify-center text-center opacity-80 relative z-10">
                                <div className="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center mb-4 text-blue-600">
                                    <MessageSquare className="w-8 h-8" />
                                </div>
                                <h4 className="text-xl font-bold text-slate-800 mb-2">Góc nhìn cá nhân</h4>
                                <p className="text-slate-500 font-medium max-w-[250px]">Những dòng tâm sự của bạn sẽ là nguồn tham khảo vô giá cho các bạn học sinh đi sau.</p>
                            </div>
                        ) : activeOptionInfo ? (
                        ...
                        <div className="mt-8 pt-4 border-t border-slate-200 flex justify-between items-center text-sm font-bold relative z-10">
                            ...
                            {currentAnswer ? (
                                <span className={`flex items-center gap-1 
                                    ${currentCriterion.type === 'text' ? 'text-blue-600' :
                                      currentAnswer === 5 ? 'text-[#22c55e]' : 
                                      currentAnswer === 4 ? 'text-[#f59e0b]' : 
                                      currentAnswer === 3 ? 'text-blue-600' :
                                      currentAnswer === 2 ? 'text-orange-500' :
                                      'text-red-500'}`}>
                                    <CheckCircle2 className="w-5 h-5" /> {currentCriterion.type === 'text' ? 'Đã ghi nhận' : 'Đã chọn'}
                                </span>
                            ) : (
                                <span className="text-amber-500">{currentCriterion.required === false ? 'Có thể bỏ qua' : 'Chưa chọn'}</span>
                            )}
                        </div>
                    </div>
                </div>
